...
//리덕스 재료 준비
import {createStore} from 'redux'; // state(상태)값을 저장시키는 역할
import {Provider} from 'react-redux'; // state(상태)값을 어떤 컴포넌트와 공유, 제공할건지를 정의

// reducer : store에 접근해서 state에 관련된 작업을 처리할 기능
// --> 현재의 상태값, 작업(action)
function reducer(currentState, action) {  
  if(currentState===undefined) {  // reducer가 처음 store에 적용될 때 default상태값을 만드는 부분
    return {
  ...
  return {...currentState};  // ... --> 객체에 대한 모든 정보를 가져오겠다
}
